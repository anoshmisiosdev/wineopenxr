/*
 * Vendored from upstream Wine include/wine/debug.h with the DECLSPEC_EXPORT
 * shim below. MinGW's winnt.h does not provide DECLSPEC_EXPORT. Wine uses it
 * to annotate the __wine_dbg_* declarations. The shim collapses them to
 * no-ops at PE compile time. Do not hand-edit. Regenerate from upstream if
